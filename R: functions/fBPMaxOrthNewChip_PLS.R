fBPMaxOrthNewChip_PLS<-function(Y,r1,angle=TRUE, version = "cov")   # r1 = response variable
{
...
numbal=ncol(Y)-1
  ...
  res<-fBalChipman_PLS(Y,r1,angle=angle,version=version)   # NOTE: "fBalChipman_PLS" for PLS PBs
  B<-res$bal
  V<-res$varbal
 # if necessary GO UP to complete
  if (sum(B==0)>0){  
    res<-fBPUpChi_PLS(Y,r1,B,version = version)    # NOTE: "fBPUpChi_PLS" for PLS PBs
  B=rbind(B,res$bal)
 ...
 # GO DOWN from numerator of the first optimal balance
  if(sum(usenum)>1){
    resP<-fBPMaxOrthNewChip_PLS(Y[,usenum],r1,angle=angle, version = version) # NOTE: "fBPMaxOrthNewChip_PLS" for PLS PBs
...
# GO DOWN from denominator of the first optimal balance
  if(sum(useden)>1){
    resP<-fBPMaxOrthNewChip_PLS(Y[,useden],r1,angle=angle, version = version) # NOTE: "fBPMaxOrthNewChip_PLS" for PLS PBs
    Bx<-matrix(0,length(resP$varbal),numpart)
    Bx[,useden]<-resP$bal
    B<-rbind(B,Bx)
    V<-cbind(V,resP$varbal)
...
# REVISIT list of balances added GO UP so as to complete the SBP if necessary GO DOWN by the POSITIVE
   ...
   if (numbaladd > 0){
     for (k in 2:(1+numbaladd)){
       usepos=(B[k,]>0)
       if (sum(usepos)>1) {
         resP<-fBPMaxOrthNewChip_PLS(Y[,usepos],r1,angle=angle, version = version) # NOTE: "fBPMaxOrthNewChip_PLS" for PLS PBs
         Bx<-matrix(0,length(resP$varbal),numpart)
         Bx[,usepos]<-resP$bal
         B<-rbind(B,Bx)
         V<-cbind(V,resP$varbal)
